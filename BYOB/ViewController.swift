...
class ViewController: UIViewController {
    ...
    func setDaysRemainingLabel() {
        ...
        var remaining = StaticFunctions.getTotalMaxBudget() - StaticFunctions.getTotalAmountSpent()
        if getDaysRemaining() != "0" {
            remaining = remaining / Double(getDaysRemaining())!
            let dailyRemainingString = formatter.string(from: NSNumber(value: remaining))
            daysRemainingLabel.text = "There are " + getDaysRemaining() + " days left in " + getCurrentMonth() + " (\(dailyRemainingString!) per day)"
        } else {
            let dailyRemainingString = formatter.string(from: NSNumber(value: remaining))
            daysRemainingLabel.text = "This is the last day of the month. (\(dailyRemainingString!) remaining)"
        }
...
//        }
        let skipBulletin = UserDefaults.standard.bool(forKey: "bulletin")
        if !skipBulletin {
    ...
    func setupBulletinBoard() {
        ...
        UserDefaults.standard.set(true, forKey: "bulletin")
